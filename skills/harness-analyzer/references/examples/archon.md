...
└────────────────────────┬───────────────────────────────────┘
                         ...
                         ▼
┌────────────────────────────────────────────────────────────┐
│  AI 작업 루프 — AI가 "완료"를 선언할 때까지 반복             │
│  orchestrator-agent.ts:846-938                             │
│                                                            │
│  ① AI가 텍스트 답변을 보냄                                  │
│     → 플랫폼에 실시간 전달 (stream) 또는 모아서 전달 (batch)  │
│     → 동시에 내부 버퍼에도 누적                              │
│                                                            │
│  ② AI가 도구 사용 요청 (파일 읽기, bash 실행, 코드 수정 등)   │
│     → 도구 실행 → 결과를 AI에게 돌려줌 → AI가 다시 판단     │
│     → 테스트 실패? → 원인 파악 → 수정 → 재실행 (루프)        │
│                                                            │
│  ③ AI가 완료 선언                                           │
│     → 세션 ID를 DB에 저장하고 루프 종료                      │
│                                                            │
│  [동시 감시] 버퍼에서 /invoke-workflow 키워드 발견되면?       │
│     → 이미 보낸 텍스트 취소 (emitRetract)                    │
│     → 워크플로우 실행으로 경로 전환                          │
└──────────────────────────┬─────────────────────────────────┘
                           │
           ┌───────────────┴───────────────┐
...
**AI 작업 루프**는 에이전트 동작의 핵심이다. AI는 한 번에 전체 답을 내놓지 않는다 — 텍스트를 쓰다가, 파일을 읽어보고, 코드를 실행해보고, 결과를 확인한 뒤 다시 판단하는 사이클을 스스로 "완료"라고 판단할 때까지 반복한다. 이 반복 구조 덕분에 "테스트를 돌렸더니 실패했다 → 원인을 분석한다 → 코드를 고친다 → 다시 테스트한다" 같은 실제 개발 흐름이 사람 개입 없이 이뤄진다.
